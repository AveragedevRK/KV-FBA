...
const DashboardCard: React.FC<DashboardCardProps> = ({ title, value, subtext, icon, trend }) => {
  return (
    <div className="bg-[var(--bg-1)] border border-[var(--border-1)] p-4 flex flex-col justify-between h-[140px] hover:border-[var(--border-2)] transition-all duration-300 group relative overflow-hidden hover:-translate-y-1 hover:shadow-lg">
      
      <div className="flex justify-between items-start">
        <h3 className="text-[14px] font-semibold text-[var(--text-secondary)] uppercase tracking-wide group-hover:text-[var(--text-primary)] transition-colors">{title}</h3>
      ...
      <div className="mt-auto">
        <div className="text-[32px] font-light text-[var(--text-primary)] leading-none mb-1">
          {value}
        </div>
        {subtext && (
